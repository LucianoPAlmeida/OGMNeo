...
class OGMNeoNode {
    ...
    */
    static addLabelForNode(label, nodeId) {
        if (_.isInteger(nodeId)) {
            return new Promise((resolve, reject) => {
                this.addLabelForNodes(label, [nodeId]).then((results) => {
    ...
    /**
        ...
        * @returns {Promise.<object|Error>} Node(if node exists) or null(if not exists) if fulfilled, some error if rejected.
    */
    static removeLabelForNode(label, nodeId) {
        if (_.isInteger(nodeId)) {
            return new Promise((resolve, reject) => {
                this.removeLabelForNodes(label, [nodeId]).then((results) => {
                    resolve(_.first(results));
                }).catch((error) => {
                    reject(error);
                });
    ...
    */
    static addLabelForNodes(label, nodesIds) {
        if (_.isString(label)) {
            return new Promise((resolve, reject) => {
    ...
    /**
        ...
        * @returns {Promise.<array|Error>} Nodes(if nodes exists) or null(if not exists) if fulfilled, some error if rejected.
    */
    static removeLabelForNodes(label, nodesIds) {
        if (_.isString(label)) {
            return new Promise((resolve, reject) => {
                this._validateAndBuildParams(nodesIds).then((params) => {
                    if (params) {
