...
class OGMRelation {
    ...
    */
    ...
    /**
        ...
        * @param {OGMNeoRelationQuery} query - Query filters.
    ...
    */
    ...
    /**
        ...
        * @param {OGMNeoWhere|OGMNeoQuery} [propertiesFilter=null] - OGMNeoWhere or OGMNeoQuery object filter to be applied on find.
    ...
    static find(nodeId, otherNodeId = null, name = null, propertiesFilter = null) {
        return new Promise((resolve, reject) => {
            if (_.isInteger(nodeId)) {
                ...
                let cypher = `${cypherMatch} RETURN r ${this._findComplement(propertiesFilter)}`;
                let session = OGMNeo.session();
    ...
    */
    static findPopulated(nodeId, otherNodeId = null, name = null, propertiesFilter = null) {
        return new Promise((resolve, reject) => {
            if (_.isInteger(nodeId)) {
                let cypherMatch = this._cypherMatch(nodeId, otherNodeId, name, propertiesFilter);
                let cypher = `${cypherMatch} RETURN n1, n2, r ${this._findComplement(propertiesFilter)}`;
                let session = OGMNeo.session();
                let readTxResultPromise = session.readTransaction(transaction => transaction.run(cypher));
                readTxResultPromise.then((result) => {
                    session.close();
    ...
    */
    ...
    static _filterQuery(propertiesFilter) {
        if (propertiesFilter) {
            if (propertiesFilter instanceof OGMNeoWhere) {
                return propertiesFilter.clause.replace('n.', 'r.');
            } else if (propertiesFilter instanceof OGMNeoQuery) {
                return (propertiesFilter.whereObject) ? propertiesFilter.whereObject.clause.replace('n.', 'r.') : ''; 
            }
        } 
        return '';
    }
    ...
    static _findComplement(query) {
        if (query && query instanceof OGMNeoQuery) {
            return ` ${query.orderByClause('r')} ${query.limitClause()}`;
        }
        return '';
    }
